...
{
    closeDevice();
    qDebug() << "RadioController:" << "Usuwanie CRadioController";
}

void CRadioController::closeDevice()
{
    qDebug() << "RadioController:" << "Zakończenie pracy urządzenia";
...
void CRadioController::setDeviceParam(QString param, int value)
{
    if (param == "biastee") {
        ...
        qDebug() << "Nieprawidłowy parametr urządzenia: " << param;
    }
...
void CRadioController::play(QString channel, QString title, quint32 service)
{
    ...
    qDebug() << "RadioController:" << "Graj:" << title << serialise_serviceid(service) << "w kanale" << channel;
    ...
    if (currentChannel != Channel || Force == true) {
        if (device && device->getID() == CDeviceID::RAWFILE) {
            ...
            if(currentFrequency != 0 && device) {
                qDebug() << "RadioController: Dostrojenie do kanału" <<  Channel << "->" << currentFrequency/1e6 << "MHz";
...
void CRadioController::setManualChannel(QString Channel)
{
    ...
    qDebug() << "RadioController: Dostrojenie do kanału" <<  Channel;
...
{
    qDebug() << "RadioController:" << "Rozpocznij skanowanie kanałów";
    ...
    else
    {
        ...
        isChannelScan = true;
        stationCount = 0;
        currentTitle = tr("Skanowanie ") + " ... " + Channel
                + " (" + QString::number((1 * 100 / NUMBEROFCHANNELS)) + "%)";
        emit titleChanged();

        currentText = tr("Znaleziono kanałów") + ": " + QString::number(stationCount);
        emit textChanged();
...
{
    qDebug() << "RadioController:" << "Skanowanie kanałów zatrzymane";
...
void CRadioController::setAGC(bool isAGC)
{
    ...
    if (device) {
        ...
        if (!isAGC) {
            ...
            qDebug() << "RadioController:" << "Autoregulacja wzmocnienia: wyłączona";
        }
        else {
            qDebug() << "RadioController:" <<  "Autoregulacja wzmocnienia: włączona";
        }
    }
...
void CRadioController::initialise(void)
{
    ...
    if(isAutoPlay) {
        play(autoChannel, tr("Włączona ostatnio wybrana stacja"), autoService);
    }
}

void CRadioController::resetTechnicalData(void)
{
    currentChannel = tr("Nie znaleziono");
    emit channelChanged();
...
void CRadioController::deviceRestart()
{
    ...
    if(!isPlay) {
        qDebug() << "RadioController:" << "Urządzenie radiowe jest niegotowe lub nie istnieje.";
        emit showErrorMessage(tr("Urządzenie radiowe jest niegotowe lub nie istnieje."));
        return;
    }
...
void CRadioController::ensembleLabel(DabLabel& label)
{
    ...
    qDebug() << "RadioController: Etykieta:" << newLabel;
...
void CRadioController::labelTimerTimeout()
{
    if (radioReceiver and not pendingLabels.empty()) {
        ...
        if (not label.empty()) {
            ...
            qDebug() << "RadioController: Znaleziono: " << qPrintable(QString::number(sId, 16).toUpper()) << qlabel;
...
void CRadioController::serviceId(quint32 sId)
{
    if (isChannelScan == true) {
        stationCount++;
        currentText = tr("Znaleziono kanały") + ": " + QString::number(stationCount);
        emit textChanged();
    }
...
void CRadioController::onNewAudio(std::vector<int16_t>&& audioData, int sampleRate, const std::string& mode)
{
    ...
    if (audioSampleRate != sampleRate) {
        qDebug() << "RadioController: Częstotliwość próbkowania audio" <<  sampleRate << "Hz, tryb=" <<
...
{
    qDebug() << "Błędna długość X-PAD, oczekiwano:" << announced_xpad_len << " otrzymano:" << xpad_len;
}
